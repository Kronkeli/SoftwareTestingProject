...
describe('toFinite function Tests', function() {
    ...
    it("Testing a strings", function() {
        res = toFinite("Infinity")
        expectedRes = 1.7976931348623157e+308;
        chai.assert.equal(res, expectedRes, "String 'Infinity'")

        res = toFinite("Number.MIN_VALUE")
        expectedRes = Number.MIN_VALUE;
        chai.assert.equal(res, expectedRes, "String 'Number.MIN_VALUE'")
    })
});
...
describe('toString function Tests', function() {
    it('Testing examples', function() {
        res = toString(null)
        expectedRes = ''
        chai.assert.equal(res, expectedRes, "Expecting null to give ''")

        res = toString(-0)
        expectedRes = '-0'
        chai.assert.equal(res, expectedRes, "Expecting -0 to give '-0'")

        res = toString([1,2,3])
        expectedRes = '1,2,3'
        chai.assert.equal(res, expectedRes, "Expecting [1,2,3] to give '1,2,3'")
    })

    it('Testing Infinity', function() {
        res = toString(Infinity)
        expectedRes = 'Infinity'
        chai.assert.equal(res, expectedRes, "Expecting Infinity to give 'Infinity'")
    })
    ...
    it('Testing JS-Object', function() {
        ...
        expectedRes = '{a:1, b:2}'
        chai.assert.equal(res, expectedRes, "Expecting {a:1, b:2} to give '{a:1, b:2}'")
    })

    it('Testing function', function() {
        res = toString(() => 1+2)
        expectedRes = '() => 1+2'
        chai.assert.equal(res, expectedRes, "Expecting () => 1+2 to give '() => 1+2'")
    })
